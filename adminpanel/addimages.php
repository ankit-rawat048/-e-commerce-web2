...
if (isset($_POST['rename'])) {
    ...
    if (!str_ends_with($newName, ".$ext")) {
        $newName .= ".$ext"; // ensure extension stays the same
    }
...
<head>
  ...
  <title>Ayurveda - Image Upload</title>
...
<body class="bg-gray-100 min-h-screen">
  ...
  <div class="flex">
    ...
    <div class="flex-1 p-6">
      ...
      <h1 class="text-2xl font-bold mb-4">Add Images</h1>
      ...
      <!-- Rename Modal -->
      <div id="renameModal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
        <div class="bg-white rounded-lg shadow-xl p-6 w-96">
          <h2 class="text-2xl font-semibold mb-4 text-gray-800">Rename Image</h2>
          <form method="POST">
            <input type="hidden" name="oldName" id="oldName">
            <input type="text" name="newName" id="newName" class="border border-gray-300 rounded px-3 py-2 w-full mb-4"
              required>
            <div class="flex justify-end gap-3">
              ...
              <button type="submit" name="rename"
                class="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600 transition">Rename</button>
            </div>
          </form>
        </div>
      </div>
