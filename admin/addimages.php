...
function getImageUrl($filename) {
    ...
    return $protocol . "://" . $host . $baseDir . "/uploads/" . $filename;
}
...
if (isset($_GET['delete'])) {
    try {
        ...
        if ($stmt->rowCount() > 0 && file_exists($fileToDelete)) {
            unlink($fileToDelete);
        ...
        $msg = "Error deleting image: " . htmlspecialchars($e->getMessage());
    }
}
...
if (isset($_POST['update'])) {
    try {
        ...
        if (isset($_FILES['newImage']) && $_FILES['newImage']['error'] === 0) {
            ...
            if (in_array($extNew, $allowed)) {
                ...
                if (move_uploaded_file($_FILES['newImage']['tmp_name'], $newPath)) {
                    ...
                    // Update database
                    $stmt = $pdo->prepare("UPDATE images SET filename = ?, url = ? WHERE filename = ?");
                    $stmt->execute([$newName, getImageUrl($newName), $oldName]);
                } else {
        ...
        $msg = "Error updating image: " . htmlspecialchars($e->getMessage());
    }
}
    ...
    try {
        ...
        if (in_array($fileType, $allowed)) {
            if (move_uploaded_file($file['tmp_name'], $targetFile)) {
                ...
                $stmt->execute([$filename, getImageUrl($filename)]);
                header("Location: " . $_SERVER['PHP_SELF']);
                exit();
            } else {
        ...
        $msg = "Error saving image to database: " . htmlspecialchars($e->getMessage());
    }
}

// Get all uploaded images from database
try {
    $stmt = $pdo->query("SELECT filename, url FROM images ORDER BY created_at DESC");
    $images = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    $msg = "Error fetching images: " . htmlspecialchars($e->getMessage());
}
?>
