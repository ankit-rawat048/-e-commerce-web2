...
<form id="cartForm" method="post">
  <div class="space-y-6" id="cartItems">
    <?php foreach ($_SESSION['cart'] as $index => $item): ?>
      <div class="flex items-center bg-white p-4 rounded shadow justify-between cart-row" data-index="<?= $index ?>" data-price="<?= $item['price'] ?>">
        <!-- Product Info -->
        <div class="flex items-center space-x-4">
          <img src="<?= $item['image'] ?>" alt="<?= $item['name'] ?>" class="w-20 h-20 object-cover rounded">
          <div>
            <h3 class="font-semibold text-lg text-gray-800"><?= $item['name'] ?></h3>
            <p class="text-gray-600">₹<?= number_format($item['price'], 2) ?> | <?= $item['weight'] ?></p>
          </div>
        </div>
        ...
        <div class="flex items-center space-x-2">
          ...
          <input type="number" name="quantities[<?= $index ?>]" value="<?= $item['quantity'] ?>" min="1" class="border rounded px-3 py-1 w-20 text-center qty-input">
        ...
        <!-- Remove -->
        <div>
          <button type="submit" name="remove_index" value="<?= $index ?>" class="text-red-500 hover:text-red-700">
  ...
  <div class="mt-10 grid grid-cols-1 md:grid-cols-3 gap-6">
    ...
    <div class="bg-white p-6 rounded shadow">
      ...
      <div class="flex justify-between py-2 border-b">
        <span>Subtotal</span>
        <span id="subtotal">₹<?= number_format($_SESSION['subtotal'], 2) ?></span>
      </div>
      <div class="flex justify-between py-2 border-b">
        <span>Shipping fee</span>
        <span id="shipping">₹<?= number_format($_SESSION['shipping'], 2) ?></span>
      </div>
      <div class="flex justify-between py-2 font-bold">
        <span>Total</span>
        <span id="total">₹<?= number_format($_SESSION['total'], 2) ?></span>
      </div>

      <div class="space-y-3 mt-4">
        <button type="submit" name="checkout" class="w-full bg-green-600 text-white py-2 rounded hover:bg-green-700 transition">
          Proceed to Checkout
        </button>
      </div>
    </div>
