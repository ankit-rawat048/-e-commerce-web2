        ...
        <div class="gallery flex gap-6 p-4 transition-transform duration-500">
          ${cat.products.map(p => `
            <a href="product.php?id=${p.id}" 
              class="product-card bg-white border-2 border-green-800 p-4 rounded-lg shadow-lg hover:shadow-xl transition transform hover:scale-105 flex-shrink-0 w-52 sm:w-56 lg:w-60">
              <img src="${p.img}" alt="${p.name}" 
                class="w-full rounded-lg border-2 border-blue-800 h-40 object-contain mb-3">
              <hr class="border-2 border-orange-400 my-4">
              <p class="font-semibold text-center text-gray-800 text-sm md:text-base mb-2 truncate">${p.name}</p>
              <div class="flex items-center w-full mt-auto gap-2">
                        <span class="bg-gray-200 text-gray-700 p-2 w-[50%] text-center text-xs md:text-sm">Himalayan Pharmacy</span>
                        <span class="bg-orange-100 text-orange-700 p-[1rem] flex justify-center items-center w-[50%] h-[100%] text-[1rem] font-bold">₹${p.price}</span>
                    </div>
            </a>
          `).join("")}
        </div>
